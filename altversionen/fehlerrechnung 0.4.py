...
def gewichteter_mittelwert_alle(werte, fehler):
    ...
    mittelwert_gew = sum(x * g for x, g in zip(werte, gewichte)) / gewicht_summe
    unsicherheit_int = (1 / gewicht_summe) ** 0.5
    n = len(werte)
    unsicherheit_ext = (sum(g * (x - mittelwert_gew)**2 for x, g in zip(werte, gewichte))/ ((n - 1) * gewicht_summe)) ** 0.5
    if unsicherheit_ext>unsicherheit_int:
       print("Achtung, beim gewichteten Mittelwert ist die äußere Unsicherheit größer, möglicherweise wurden die Unsicherheiten unterschätzt")
       return mittelwert_gew,unsicherheit_ext
    else:
       return mittelwert_gew, unsicherheit_int
